<?php
$common__image_hover = get_field('common__image_hover');
?>

<div class="preview-solutions">
    <a href="<?php the_permalink(); ?>" class="preview-solutions__img">
        <?php
            getPostThumbnail(get_the_ID(), 'img img--default');

            if ($common__image_hover) {
                echo wp_get_attachment_image($common__image_hover, 'full', null, ['class' => 'img img--hover']);
            }
            else {
                getPostThumbnail(get_the_ID(), 'img img--hover');
            }
        ?>
    </a>

    <div class="preview-solutions__body">
        <a href="<?php the_permalink(); ?>" class="h4 preview-solutions__title"><?php the_title(); ?></a>

        <?php
            if (has_excerpt()) {
                ?>
                <div class="content preview-solutions__content"><?php the_excerpt(); ?></div>
                <?php
            }
        ?>
    </div>

    <a href="<?php the_permalink(); ?>" class="button button--read preview-solutions__button">
