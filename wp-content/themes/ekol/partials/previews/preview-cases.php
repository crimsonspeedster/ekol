...
<div class="preview-cases">
    <a href="<?php the_permalink(); ?>" class="preview-cases__img">
    ...
    <?php
        if (is_array($terms) && !empty($terms)) {
            ?>
            <div class="tags preview-cases__tags">
                <?php
                    foreach ($terms as $term) {
                        ?>
                        <a href="<?= get_term_link($term); ?>" class="tags-item"><?= $term->name; ?></a>
                        <?php
                    }
                ?>
            </div>
    ...
    <div class="preview-cases__body">
        <div class="preview-cases__content">
            <a href="<?php the_permalink(); ?>" class="h4 preview-cases__title"><?php the_title(); ?></a>

            <?php
                if (has_excerpt()) {
                    ?>
                    <div class="content preview-cases__description"><?php the_excerpt(); ?></div>
                    <?php
                }
            ?>
        </div>

        <a class="preview-cases__button" href="<?php the_permalink(); ?>">
